class openstack::horizon inherits openstack {
    if str2bool($https) {
        ...
        ->
        exec { 'generate_tls':
                command => "/usr/bin/openssl req -new -newkey rsa:2048 -days 3650 -nodes -x509 -keyout /opt/openstack.key -out /opt/openstack.crt -subj \"/C=FR/ST=Pays de la Loire/L=Nantes/O=GenoBiRD/OU=IT BiRD/CN=${os_public_name}\"",
        ...
        exec { 'add_cert_to_trust_store':
                command => '/usr/bin/cp /opt/openstack.crt /etc/pki/ca-trust/source/anchors/openstack.crt',
